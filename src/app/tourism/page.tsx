...
export default function Home() {
  ...
  const [studentNospy, setStudentNospy] = useState("0");
  ...
  const fetchResults = async () => {
    ...
    try {
      const response = await fetch("/api/proxy/tourism", {
        method: "POST",
  ...
  const Specialties = [
    { name: 'بدون اختصاص', value: 0 },
    { name: 'السياحة', value: 1 },
    { name: 'الإدارة الفندقية', value: 2 },
  ]
  return (
    <div className="bg-gray-900 text-gray-100 font-droid font-cairo relative">
      ...
      <main className="container mx-auto px-4 pt-16 pb-8 ">
        <h1 className="text-center text-lg font-bold mb-5">كلية السياحة - جامعة حمص </h1>
